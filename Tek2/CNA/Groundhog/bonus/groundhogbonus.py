...
def get_weirdest_values(values_entered: List[float], nb_days: int, f_inpout: float, weirdest_values: List[float]):
    weird_array: list = list()
    last_period: int = len(values_entered) - nb_days
    for i in range(last_period, len(values_entered)):
        weird_array.append(values_entered[i])
    weird_array.sort()
    q1: float = weird_array[len(weird_array) // 4 - int(len(weird_array) / 4)]
    q3: float = weird_array[3 * len(weird_array) // 4 - int(len(weird_array) / 4)]
    interval: float = q3 - q1
    interval_limit: float = 0.8 * interval
    interval_limit_sup: float = q1 - interval_limit
    interval_limit_inf: float = q3 + interval_limit
...
def get_top_weirdest_values(values_entered: List[float], weirdest_values: List[float]):
    ...
    if len(final_list) < 5:
        while len(final_list) < 5:
            final_list.append(random.choice(values_entered))
    return final_list
...
def read_input(nb_switched: int, values_entered: List[float], weirdest_values: List[float], has_done_period: bool) -> float:
    try:
        ...
        sys.exit(84)
    if inpout == "STOP":
        five_weird = get_top_weirdest_values(values_entered, weirdest_values)
        ypoints = np.array(values_entered, dtype=np.float32)
        y_weirdest = np.array(five_weird, dtype=np.float32)
        x_weirdest = np.array([np.where(ypoints == y_weirdest[0])[0],
                               np.where(ypoints == y_weirdest[1])[0],
                               np.where(ypoints == y_weirdest[2])[0],
                               np.where(ypoints == y_weirdest[3])[0],
                               np.where(ypoints == y_weirdest[4])[0]]) 
        plt.plot(ypoints)
        plt.plot(x_weirdest, y_weirdest, 'o')
        plt.title("Temperatures and top  outliers")
