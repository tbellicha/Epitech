...
packet_result_t handle_get_comments_packet(peer_t *peer, size_t length)
{
    ...
    if (!peer_check_logged_in(peer, length))
        return PRESULT_OK;
    if (length != 3 * sizeof(uuid_t)) {
        reply_error(peer, REPLY_INVALID_LENGTH,
            "channel list packet should contain three uuids", length);
        return PRESULT_OK;
    }
    my_cbuffer_pop_front(&peer->in_buf, &team_id, sizeof(uuid_t));
    my_cbuffer_pop_front(&peer->in_buf, &channel_id, sizeof(uuid_t));
    my_cbuffer_pop_front(&peer->in_buf, &thread_id, sizeof(uuid_t));
    reply_comment_list(peer, team_id, channel_id, thread_id);
    return PRESULT_OK;
}
